class Zen_ArtillerySmokeGrenade : M18SmokeGrenade_ColorBase
{
	// Taken from ContaminatedArea_Dynamic.c
	int m_BombCount = 0;

	// Set color
	void Zen_ArtillerySmokeGrenade()
	{
		// Set explosive artillery colour
	...
	{
		super.OnWorkStart();
		GetGame().GetCallQueue(CALL_CATEGORY_SYSTEM).CallLater(StartBombs, Math.RandomInt(GetZenArtilleryConfig().MinStrikeSecs * 1000, GetZenArtilleryConfig().MaxStrikeSecs * 1000), false, this);
	}
	...
	{
		//ZenFunctions.SendGlobalMessage("START BOMBS!");
